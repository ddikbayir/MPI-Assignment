...
int solve(ofstream& logfile, double ***_Ew, double ***_E, double ***_E_prev, double **R, int m, int n, int niters, double alpha, double dt, int plot_freq, Plotter *plotter, int stats_freq, int px, int py,int allm = -1, int alln = -1, bool noComm= false){
 ...
 char * sendbuffer2 = new char[col_msgsiz];
 char * recvbuffer2 = new char[col_msgsiz];
   ...
   for (j=1; j<=m+1; j++){
	...
	// Leftmost
		if (colid == 0) {
			...
			MPI_Isend(sendbuffer, col_msgsiz, MPI_PACKED,rank+1,NEIGHBOR_C, MPI_COMM_WORLD, &send_request);
			// Recieve packed array
			position=0; // Reset position
			MPI_Irecv(recvbuffer, col_msgsiz, MPI_PACKED, rank+1,NEIGHBOR_C, MPI_COMM_WORLD, &recv_request);
		...
		//middle
			cout << "\tRank " << rank << " inside middle col. passing\n";
			int position=0;
			int position2=0;
			for (int j=1; j<=m+1; j++){
				/* Packing to send right */
			    MPI_Pack(&E_prev[j][n+1],1, MPI_DOUBLE, sendbuffer, col_msgsiz, &position, MPI_COMM_WORLD);
				/* Packing to send left */
			    MPI_Pack(&E_prev[j][1],1, MPI_DOUBLE, sendbuffer2, col_msgsiz, &position2, MPI_COMM_WORLD);
			}
			// 1st communication
			MPI_Isend(sendbuffer, col_msgsiz, MPI_PACKED,rank+1,NEIGHBOR_C, MPI_COMM_WORLD, &send_request);
			// Recieve packed array
			...
			// 2nd communication			
			MPI_Isend(sendbuffer2, col_msgsiz, MPI_PACKED,rank-1,NEIGHBOR_C, MPI_COMM_WORLD, &send_request2);
			// Recieve packed array
			position2=0; // Reset position
			MPI_Irecv(recvbuffer2, col_msgsiz, MPI_PACKED, rank-1,NEIGHBOR_C, MPI_COMM_WORLD, &recv_request2);
			...
			for (int j=1;j<=m+1;j++) {
				  MPI_Unpack(recvbuffer, col_msgsiz, &position, &E_prev[j][n+2], 1, MPI_DOUBLE, MPI_COMM_WORLD);
				  MPI_Unpack(recvbuffer2, col_msgsiz, &position2, &E_prev[j][0], 1, MPI_DOUBLE, MPI_COMM_WORLD);
			}
   ...
   if (plot_freq){
		...
		* needs to pack up it's own working segment and send it to thread 0 */
		...
		if (rank != root) {	
			...
			MPI_Send(msgbuffer, plot_msgsiz, MPI_PACKED,root,PLOT, MPI_COMM_WORLD);
		}
		else { 
			// IN THREAD 0
